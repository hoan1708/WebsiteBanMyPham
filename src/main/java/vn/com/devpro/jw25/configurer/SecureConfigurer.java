...
import vn.com.devpro.jw25.service.UserDetailsServiceImpl;
...
public class SecureConfigurer extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		...
		http.csrf().disable().authorizeRequests() // bat dau cau hinh: tat ca cac requests tu
				...
				.antMatchers("/customer/**", "/FileUpload/**", "/admin/**", "/user/**", "/login", "/logout")
				.permitAll()
...
//				 .antMatchers("/manager/**").hasAnyAuthority("ADMIN") //CHI ROLE ADMINISTRATOR
				// MOI VAO DUOC CAC TRANG ADMINISTRATOR
...
//				.defaultSuccessUrl("/manager/home-admin", true) // login thanh cong thi tro ve trang home.jsp
				 ...
				 .successHandler(new UrlAuthenticationSuccessHandler())  //  chuyển đến request phù hợp với Role 
	...
	//Cau hinh phan dang nhap bang tai khoan trong DB
		...
		public static void main(String[] args) {
			System.out.println(new BCryptPasswordEncoder(4).encode("user"));
